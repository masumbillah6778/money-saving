...
const ADMIN_PASSWORD_KEY="money_saving_admin_personal_password";
const ADMIN_EMAIL="masumbillah6778@gmail.com";
...
function getAdminPassword(){return localStorage.getItem(ADMIN_PASSWORD_KEY)||""}
function setAdminPassword(p){localStorage.setItem(ADMIN_PASSWORD_KEY,p)}
function isAdmin(){return localStorage.getItem(ADMIN_SESSION_KEY)==="1"}
function hideAdminPasswordBoxes(){
  ["adminPasswordBox","adminSetupBox","adminResetBox"].forEach(id=>$(id)?.classList.add("hidden"));
}
function showAdminSetup(){
  hideAdminPasswordBoxes();
  $("adminGoogleBtn").classList.add("hidden");
  $("adminSetupBox").classList.remove("hidden");
  $("adminNewPassword").value="";$("adminConfirmPassword").value="";$("adminNewPassword").focus();
}
...
function adminLogin(){
  // Existing Google-admin gate is preserved; the new step is the separate app password.
  if(getAdminPassword()) showAdminPasswordLogin(); else showAdminSetup();
}
function enterAdmin(){
...
function saveAdminPassword(){
  const p=$("adminNewPassword").value, c=$("adminConfirmPassword").value;
  if(p.length<4)return alert("Password কমপক্ষে ৪ অক্ষরের দিন।");
  if(p!==c)return alert("Password দুটি একই হয়নি।");
  setAdminPassword(p);enterAdmin();alert("✅ Admin Personal Password সেট হয়েছে।");
}
function verifyAdminPassword(){
  const p=$("adminPasswordInput").value;
  if(!p)return alert("Admin Personal Password দিন।");
  if(p!==getAdminPassword())return alert("❌ Admin Personal Password সঠিক নয়।");
  enterAdmin();
}
function showAdminReset(){
  hideAdminPasswordBoxes();
  $("adminGoogleBtn").classList.add("hidden");
  $("adminResetBox").classList.remove("hidden");
  $("adminResetEmail").value="";$("adminResetNewPassword").value="";$("adminResetConfirmPassword").value="";$("adminResetEmail").focus();
}
function hideAdminReset(){showAdminPasswordLogin()}
function resetAdminPassword(){
  const email=$("adminResetEmail").value.trim().toLowerCase();
  const p=$("adminResetNewPassword").value,c=$("adminResetConfirmPassword").value;
  if(email!==ADMIN_EMAIL)return alert("❌ Admin Gmail সঠিক নয়।");
  if(p.length<4)return alert("Password কমপক্ষে ৪ অক্ষরের দিন।");
  if(p!==c)return alert("Password দুটি একই হয়নি।");
  setAdminPassword(p);alert("♻️ Admin Personal Password Reset হয়েছে। এখন নতুন Password দিয়ে Login করুন।");showAdminPasswordLogin();
}
window.adminLogin=adminLogin;window.verifyAdminPassword=verifyAdminPassword;window.saveAdminPassword=saveAdminPassword;window.showAdminReset=showAdminReset;window.hideAdminReset=hideAdminReset;window.resetAdminPassword=resetAdminPassword;
...
function renderUsers(){
  ...
  const listTotal = list.reduce((sum,u) => sum + userTotal(u.uid), 0);
  $("usersPrintTotal").textContent = money(listTotal);
}
...
function renderDetails(uid){
  ...
  $("detailPrintTotal").textContent=money(run);
}
...
function printSelectedUser(){
  ...
  document.body.classList.add("printing-selected-user");
  const cleanup=()=>document.body.classList.remove("printing-selected-user");
...
function init(){
  // A page reload ends the Admin app session; the Personal Password is still required on the next login.
  localStorage.removeItem(ADMIN_SESSION_KEY);
  $("login").classList.remove("hidden");$("app").classList.add("hidden");$("menuBtn").classList.add("hidden");
  hideAdminPasswordBoxes();$("adminGoogleBtn").classList.remove("hidden");
}
init();
