...
async function ensureProfile(){
  ...
  setStatus('Firebase profile পরীক্ষা করা হচ্ছে...');
  const snap=await getDoc(ref);

  if(snap.exists()){
    profile={uid:currentUser.uid,...snap.data()};
  }else{
    setStatus('নতুন User profile তৈরি করা হচ্ছে...');
    const id=await makeMemberId();
    profile={
      uid:currentUser.uid,
      memberId:id,
      displayName:'',
...
async function loadDeposits(){
  setStatus('আপনার জমার তথ্য লোড হচ্ছে...');
  const ref=collection(db,'users',currentUser.uid,'deposits');
  let snap;
  try{
    snap=await getDocs(query(ref,orderBy('createdAt','asc')));
  }catch(orderErr){
    // Existing documents created before createdAt was added can make an
    // ordered query fail. Fall back to a simple read.
...
onAuthStateChanged(auth,async user=>{
 ...
 try{await ensureProfile();if(!profile.displayName)showSetup();else await showApp();}
    ...
    const code=e?.code||'';
    let msg=e?.message||String(e);
    if(code==='permission-denied') msg='Firestore permission denied. Firebase Rules এবং এই Project-এর Firestore path পরীক্ষা করুন।';
    setStatus('❌ '+msg,true);
    alert('User data load failed:\n\n'+msg);
    try{await signOut(auth);}catch(signErr){console.error(signErr);}
  }
});
